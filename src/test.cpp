...
TEST(LookupTable, ParrallelWriteRemoveReadValues)
{
    ...
    std::thread thread1([&table, &thread1_is_done]()
    {
        ...
        std::atomic_store_explicit(&thread1_is_done, true, std::memory_order_relaxed);
    });
    ...
    std::thread thread2([&table, &thread1_is_done]()
    {
        ...
        for (;;)
        {
            if (std::atomic_load_explicit(&thread1_is_done, std::memory_order_relaxed))
                break;
            for (int i = 0; i < iterations; ++i)
            {
                int idx = i;
                table.add_or_update(idx, "AAAAAAA = " + std::to_string(idx));
                table.remove(idx - step);
            }
        }
    });
